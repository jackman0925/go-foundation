...
- 明确 `netx` 网卡名称判断与网络可达性的边界，补充 Windows vEthernet 候选收集示例及回归测试，保持默认过滤行为不变。

## [0.1.6] - 2026-09-07

- 新增 `netx.ShutdownHTTPServer`，优先优雅关闭 HTTP 服务；超时或取消后强制关闭活动连接，避免进程退出被在途请求阻塞。

## [0.1.5] - 2026-08-26

- 新增 `lockx` 包，提供仅在单个进程内生效的按键互斥锁。
- 支持 `Lock` 上下文取消、`TryLock` 非阻塞获取和幂等解锁。
- 同一键等待者按队列逐个唤醒，取消的等待者会被清理；无等待者时自动清理锁键状态。
- 明确不提供分布式锁、重入、自动续期和全局默认实例，保持基础库边界清晰。
- 补充锁竞争、取消等待、重复解锁、不同键并行和状态清理等单元测试。

## [0.1.4] - 2026-08-18

- 增强 `timex` 包，新增斜杠、紧凑日期和短年份日期时间格式常量。
- `timex.ParseTime` 支持 `2006/01/02 15:04:05`、`20060102150405`、`2006/01/02`、`20060102` 等常见格式。
- 新增 `ParseTimeInLocation`、Unix 毫秒时间戳转换、日期加减、日期比较、秒数差值和年龄计算工具。
- 新增周边界计算、中文星期、ISO 星期数字和毫秒级日期时间格式化工具。
- 补充 `timex` 单元测试，覆盖时区保留、格式解析、日期比较、年龄边界、星期转换和周边界。
